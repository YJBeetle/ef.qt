...
const generate = (files) => {
	...
	if(files.length==1)
	{
		if(files[0].nameSpace)
			ifndef += `_${files[0].nameSpace}`;
		if(files[0].className)
			ifndef += `_${files[0].className}`;
	}
	return `// Generated by ef.qt on ${(new Date()).toDateString()}

#ifndef EFQT_GENERATED_HPP${ifndef}
#define EFQT_GENERATED_HPP${ifndef}
...
#endif // EFQT_GENERATED_HPP${ifndef}
`
}
